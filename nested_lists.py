...
NOTE:
    If there are multiple students with the second lowest grade, order their names alphabetically and print each
    name on a new line.

EXAMPLE:
    a. records = [["chi", 20.0], ["beta", 50.0], ["alpha", 50.0]]
    b. The ordered list of scores is [20.0, 50.0], so the second lowest score is 50.0. There are two students with that
...
"""
EXPLANATION:
    a. Creates a new list that represents a sorted version of 'score_only'
    b. Eliminates duplicates (by making this list into into a set)
    c. Chooses the second highest float in the list (in position [1])
    d. Iterates through a sorted version of the list containing both names and scores
    e. If the loop finds a score that matches the output of 'second_highest', it prints the associated name
"""
